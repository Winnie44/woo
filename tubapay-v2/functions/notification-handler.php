...
if ($metadata->commandType == 'TRANSACTION_STATUS_CHANGED') {
    ...
    $paymentScheduleId = $transaction->requestPositions[0]->paymentScheduleId;
    $args = array(
        'meta_key'      => 'tubapay2_paymentScheduleId',
        'meta_value'    => $paymentScheduleId,
        'meta_compare'  => '=',
        'return'        => 'ids'
    );
    $same_orders = wc_get_orders( $args );

    if (!$order) {
        http_response_code(400);
        die("missing order number: ".esc_html($order_id)." ");
    } else {
        if (!empty($same_orders)) {
            http_response_code(400);
            die("already existing order paymentScheduleId: ".esc_html($paymentScheduleId)." ");
        }
        ...
        $partial_order->update_meta_data( 'tubapay2_paymentScheduleId', $paymentScheduleId );
        
        $mainorder_meta = $order->get_meta_data();
        foreach ($mainorder_meta as $meta) {
            if (strpos($meta->key, 'vat_number') !== false || strpos($meta->key, 'nip') !== false) {
                $partial_order->update_meta_data( $meta->key, $meta->value );
            }
        }
